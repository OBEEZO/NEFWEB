...
@php
    $pageTitle = 'Events';
    $pageDescription = 'Stay updated with our latest events and activities.';
@endphp
...
<div class="tab-class text-center wow fadeInUp" data-wow-delay="0.3s">
    @if (auth()->check() && auth()->user()->rank == 0)
        <a class="btn btn-secondary btn-sm d-inline-block w-auto px-3 py-1 mb-4" href="{{ route('event-create') }}">
        Add New Event
        </a>
    @endif
</div>
        ...
        @foreach ($events as $event)
            <div class="col-md-4 mb-4"> <!-- Each event occupies 1/3rd of the row -->
                <div class="card h-100"> <!-- Card stretches to match height -->
                    ...
                    <img 
                        class="card-img-top"
                        src="{{ $event->image ? asset('storage/' . $event->image) : asset('assets/images/no-image.png') }}"
                        alt="Event Image"
                    ...
                    <div class="card-body">
                        ...
                        <p class="card-text">
                            ...
                            {!! implode(' ', array_slice(explode(' ', $event->description), 0, 20)) !!}...
                        </p>
                        <div class="d-flex justify-content-center gap-2">
                            <a class="btn btn-primary btn-sm" href="{{ route('event-show', $event->id) }}">Read More</a>
                            @auth
                                @if (auth()->user()->is_admin)
                                    <a class="btn btn-info btn-sm text-light" href="{{ route('event-edit-form', $event->id) }}">Update</a>
                                @endif
                            @endauth
                        </div>
                    </div>
...
<script>
    $(document).ready(function() {
        $('#search-addon').on('click', function(e) {
            ...
            if (currentSegment === 'events') {
                window.location.href = 'events?q=' + searchQuery;
            }
        });
    });
</script>
